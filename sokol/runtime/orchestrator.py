        ...
        if active_task:
            ...
            if self._task_manager.is_request_related_to_task(text, active_task):
                ...
                logger.info_data(
                    ...
                    {"task_id": active_task.task_id, "goal": active_task.goal, "intent_task_related": intent.task_related},
                )
                ...
                decision_trace = DecisionTrace(
                    ...
                    input_context={"request": text, "task_id": active_task.task_id, "intent_type": intent.intent_type.value},
                    ...
                    influencing_factors=[f"Related to task: {active_task.goal}", f"Intent: {intent.intent_type.value}"],
                ...
                decision_trace = DecisionTrace(
                    ...
                    influencing_factors=["Request not related to active task", f"Intent: {intent.intent_type.value}"],
                    memory_influence={"task_goal": active_task.goal},
                )
                self._decision_trace_collector.record_decision(decision_trace, execution_id=None)  # No trace yet
